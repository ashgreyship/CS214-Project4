    ...
    while(movie){
        if(movie->empty==1){
            ...
            if(movie->director_name==NULL){
                ...
                fputs(movie->director_name,output);fputs(",",output);}
            ...
            if(movie->duration==-1){
                ...
                fprintf(output, "%d", movie->duration);fputs(",",output);
            }
            ...
            if(movie->actor_2_name==NULL){
                ...
                fputs(movie->actor_2_name,output);fputs(",",output);}
            ...
            if(movie->gross==-1){
                ...
                fprintf(output, "%d", movie->gross); fputs(",",output);}
            if(movie->generes==NULL){
                fputs(",",output);
            }else{
            ...
            if(movie->actor_1_name==NULL){
                ...
                fputs(movie->actor_1_name,output); fputs(",",output);}

            if(movie->movie_title==NULL){
                fputs(",",output);
            }else{
                fputs( movie->movie_title,output);fputs(",",output);}
            ...
            if(movie->num_voted_users==-1){
                ...
                fprintf(output, "%d", movie->num_voted_users);fputs(",",output);}
            ...
            if(movie->cast_total_facebook_likes==-1){
                ...
                fprintf(output, "%d", movie->cast_total_facebook_likes);fputs(",",output);}

            if(movie->actor_3_name==NULL){
               fputs(",",output);
            }else{
                fputs(movie->actor_3_name,output);fputs(",",output);}

            if(movie->facenumber_in_poster==-1){
                fputs(",",output);
            }
            else{
                fprintf(output, "%d", movie->facenumber_in_poster);fputs(",",output);}

            if(movie->plot_keywords==NULL){
                fputs(",",output);
            }else{
                fputs(movie->plot_keywords,output);fputs(",",output);}

            if(movie->movie_imdb_link==NULL){
                fputs(",",output);
            }else{
                fputs(movie->movie_imdb_link,output);fputs(",",output);}
            ...
            if(movie->num_user_for_reviews==-1){
               ...
               fprintf(output, "%d", movie->num_user_for_reviews);fputs(",",output);}
            if(movie->language==NULL){
                fputs(",",output);
            }else{
            ...
            if(movie->country==NULL){
                ...
                fputs(movie->country,output);fputs(",",output);}

            if(movie->content_rating==NULL){
                fputs(",",output);
            }else{
                fputs(movie->content_rating,output);fputs(",",output);}

            if(movie->budget==-1){
               fputs(",",output);
            }
            else{
                fprintf(output, "%d", movie->budget);fputs(",",output);}

            if(movie->title_year==-1){
                fputs(",",output);
            }
            else{
                fprintf(output, "%d", movie->title_year);fputs(",",output);}

            if(movie->actor_2_facebook_likes==-1){
                fputs(",",output);
            }
            else{
                fprintf(output, "%d", movie->actor_2_facebook_likes);fputs(",",output);}

            fprintf(output, "%g", movie->imdb_score);fputs(",",output);

            if(movie->aspect_ratio==-1){fputs(",",output);}
            else
            {   fprintf(output, "%g", movie->aspect_ratio);fputs(",",output);}

            if(movie->movie_facebook_likes==-1){
                fputs(",",output);
            }
            else{
                fprintf(output, "%d", movie->movie_facebook_likes);fputs("\n",output);}
